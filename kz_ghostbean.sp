...
public Action kz_line_pp(int client, int args)
{
	...
	{
		PrintToChat(client, "[NEKO KZ 引导] 当前replay 长度不需要进退");
		return Plugin_Continue;
	}
...
public Action kz_line_menu(int client, int args)
{
	...
	menu.SetTitle(" - NEKO KZ | 路径引导 - \n ");
	char buffer[64];
	Format(buffer, sizeof(buffer), "路径引导 %s", iPlayerTickData[client].replay_line_enable ? "已打开":"已关闭");
	menu.AddItem("1", buffer, g_bloadreplay ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
	Format(buffer, sizeof(buffer), "自动更新进度 %s", iPlayerTickData[client].replay_auto_path ? "已打开":"已关闭");
	menu.AddItem("1", buffer, (g_bloadreplay && iPlayerTickData[client].replay_line_enable) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
	menu.AddItem("4", "路段向前 >> \n 可绑定指令sm_lpp使用", (g_bloadreplay && !iPlayerTickData[client].replay_auto_path) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
	menu.AddItem("5", "路段倒退 << \n 可绑定指令sm_ldd使用", (g_bloadreplay && !iPlayerTickData[client].replay_auto_path) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
	menu.AddItem("1", "从头开始 << \n ", (g_bloadreplay && !iPlayerTickData[client].replay_auto_path) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);	
	if(g_bloadreplay)
		Format(buffer, sizeof(buffer), "进度 %i - %i / %i 回放在下一页", iPlayerTickData[client].replay_start_tick, iPlayerTickData[client].replay_end_tick, playbackTickData.Length);
	else
		Format(buffer, sizeof(buffer), "当前地图还未存在回放数据\n或正在第一次查询全球wr回放数据");
	menu.AddItem("4", buffer, ITEMDRAW_DISABLED);
	menu.AddItem("1", "用BOT观看这段路线", g_bloadreplay ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
	...
	switch(action)
	{
		case MenuAction_Select:
		{
			if(option == 0)
			{
				...
				PrintToChat(client, "[NEKO KZ 引导] %s", iPlayerTickData[client].replay_line_enable ? "已打开":"已关闭");
			...
			if(option == 1)
			{
				...
				PrintToChat(client, "[NEKO KZ 引导] 自动更新进度%s", iPlayerTickData[client].replay_auto_path ? "已打开":"已关闭");
				kz_line_menu(client, 0);	
			}
			if(option == 2)
			{
			...
			if(option == 4)
			{
				PrintToChat(client, "[NEKO KZ 引导] 从头开始播放replay line");
				iPlayerTickData[client].replay_start_tick = 1;
				iPlayerTickData[client].replay_post_tick = iPlayerTickData[client].replay_start_tick;
				iPlayerTickData[client].replay_end_tick = iPlayerTickData[client].replay_start_tick + Load_DataTick;
				kz_line_menu(client, 0);
